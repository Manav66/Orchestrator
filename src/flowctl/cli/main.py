...
Every command here does exactly one thing: build a DB session, call
one function in flowctl.app.application, and render the result with
rich. No command talks to flowctl.core or flowctl.storage directly.
"""
...
def register_cmd(
    ...
    schedule: Optional[str] = typer.Option(
        None, "--schedule", help="Cron expression, e.g. '0 6 * * *'. Used by the scheduler (Phase 4)."
    ),
    attr: Optional[str] = typer.Option(
        None, "--attr", help="Name of the Pipeline variable, if the file defines more than one."
    ),
    ...
    `flowctl status` and become visible to the scheduler. Registering
    an already-known pipeline updates its schedule.
    """
    session = _session()
    pipeline_row = app_layer.register(file, session, attr=attr, schedule=schedule)
    console.print(
...
def status():
    ...
    adhoc_runs = [r for r in app_layer.list_runs(session, limit=20) if r.pipeline_name not in registered_names]
    if adhoc_runs:
